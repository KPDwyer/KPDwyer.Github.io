---
layout: post
title:  "RPG Maker Autotile Pipeline in Unity"
...
#Overview
##Goal
RPG Maker has a real nice "auto tile" system, whereby you load in a fairly simple asset and when you use the tile generated by that asset to draw on the tilemap, it automatically renders each tile such that its aware of its local neighbours.

![Atlas on left, Editor Function on right]

This is generally know as Autotiling, and will be referred to as such in this article.  

I set out to duplicate the functionality of RPGMaker's autotiling - specifically what is commonly known as their A2 sheet, shown above.  I was surprised to find that while Unity accomodates the creation of these autotiles, a few layers of complexity prevent it from being as smooth as RPG Maker's UX.  
...
Now that we understand the functionality reasonably well, The most straighforward solution presents itself to us:  lets generate these 48 fullsized tiles from our pool of 20 subtiles.
